...
# 1. If discount > 10 mark "High Discount".
sales_data$DiscountLabel <- for(i in 1:nrow(sales_data)){
  if(sales_data$Discount[i] > 10){
    sales_data$DiscountLabel[i] <- "High Discount"
  } else {
    sales_data$DiscountLabel[i] <- "Normal"
  }
}
sales_data
